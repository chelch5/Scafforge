...
- [ ] State explicitly that real agents run on host-resident workers, not in the Windows app.
- [ ] Freeze backend-only dispatch of mutation work.
- [ ] Prohibit direct control-plane fallback shell execution as canonical worker behavior.

### Phase 2: Define host registration

- [ ] Define minimum host identity fields.
- [ ] Define host-kind vocabulary and capability reporting.
- [ ] Define health-state vocabulary and stale/offline handling.
- [ ] Define how worker versioning and compatibility are surfaced.

### Phase 3: Define dispatch and repo affinity

- [ ] Define how a job targets a worker host.
- [ ] Define how repo path bindings interact with host dispatch.
- [ ] Define how repos without a local Windows checkout can still be targeted on WSL or SSH Linux.
- [ ] Define how the backend responds when a repo binding exists but the host is unavailable.

### Phase 4: Define trust and operational guardrails

- [ ] Separate host trust from provider credentials and app auth.
- [ ] Define how SSH, WSL, and Windows-local trust are represented without becoming hidden mutation lanes.
- [ ] Define minimum worker evidence and progress reporting expected back to the backend.
