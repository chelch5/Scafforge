...
- [x] Audit the certified, partial, experimental, and preview tool lists and make sure docs, code, and tests agree.
- [x] Decide which tool names are too broad for the currently supported behavior and need narrower contracts.
- [x] Define when v2 is allowed to be used and when Scafforge must still target the v1 shipped API.
- [x] Record the v1-versus-v2 boundary in a structured machine-readable contract artifact inside `blender-agent`, such as `mcp-server/capability-contract.json`, not only as prose scattered across README text.
- [x] Freeze a baseline snapshot of the current maturity table and hardening priorities in this plan folder before implementation starts.
- [x] Ensure no packaging or README claim outruns the actual maturity table.

### Phase 2: Harden the server runtime and safety posture

- [x] Prioritize the server hardening tasks already called out in the repo’s own plan: runner isolation, structured result envelopes, security mode separation, and payload validation.
- [x] Improve unsafe/Python escape-hatch gating and observability.
- [x] Ensure headless runs and dry-run behavior do not depend on interactive Blender assumptions.
- [x] Make warnings, diagnostics, and partial-execution signals machine-readable.

### Phase 3: Certify the highest-value tool families

- [x] Choose the tool families Scafforge needs most: scene organization, modifiers, UVs, materials, export, render preview, and QA.
- [x] Build representative fixture coverage for each certified lane.
- [x] Downgrade or narrow any tool whose advertised category is broader than what the tests prove.
- [x] Keep deterministic naming, transforms, and object/report behavior as explicit validation targets.

### Phase 4: Strengthen QA and engine handoff

- [x] Expand `quality_validate` into profile-based QA that matches the repo’s own quality plan (`draft`, `game_asset`, `hero_asset`, `animation_export`).
- [x] Validate export presets and handoff assumptions for Godot, Unity, and Unreal against stable interchange artifacts.
- [x] Treat Unity and Unreal validation as interchange-format structural verification only by default; any real engine-runtime import testing requires a separately approved exception and must not be assumed in the base plan.
- [x] Ensure export and QA results are suitable for Scafforge to consume as evidence later.
- [x] Add screenshot/render or preview artifacts where they materially improve trust.

### Phase 5: Redesign or prune the skill layer where necessary

- [x] Compare `skills-src/` against the real tool maturity table and remove any promise the server cannot keep.
- [x] Rewrite skills as deterministic playbooks that match the certified tool contracts.
- [x] Ensure art-critique and quality skills are grounded in real QA or render surfaces, not only subjective language.
- [x] Keep packaged skills synchronized with the validated source skills.

### Phase 6: Packaging, install, and Linux/headless readiness

- [x] Define the supported installation paths for Codex, Claude Code, and Opencode clearly.
- [x] Treat other MCP hosts as out of scope for this plan unless a follow-on plan explicitly adds them.
- [x] Add or harden Linux-first/headless setup and CI notes where the repo still assumes an interactive Windows workstation.
- [x] Make sure the repo can explain what works in a fully headless Blender environment versus what still needs local GUI/runtime support.
- [x] Ensure packaging does not duplicate server code or drift between clients.
- [x] Name the concrete headless test infrastructure in `blender-agent`, such as a dedicated `mcp-server/tests/headless/` lane or equivalent, and define what a passing headless run looks like.
- [x] Require evidence from a real background Blender invocation path, such as `blender --background`-backed tests or an equivalent CI lane, before claiming Linux/headless support.
- [x] Require a concrete cross-repo handoff proof package, such as `HARDENING-PROOF.json` in the `blender-agent` repo root, with at minimum: capability table version, structured v1/v2 contract path, certified tool families, and headless-proof artifact locations.
- [x] Define the acceptable headless proof format for Scafforge handoff: either a committed structured result artifact such as `mcp-server/tests/headless/results.json` or a named CI workflow result tied back to the hardened capability table.
- [x] As a final handoff step, re-evaluate Scafforge’s Blender references against the hardened `blender-agent` capability table.
- [x] Treat Scafforge-side doc updates as blocked until `blender-agent` can point to both a post-hardening capability table and a concrete headless-validation proof artifact or CI result that matches the narrowed contract.
- [x] Verify that plan `04` already created `skills/project-skill-bootstrap/references/blender-support-matrix.md` before updating it here; if it does not exist yet, stop and resolve the plan `04` dependency first.
- [x] Update `skills/project-skill-bootstrap/references/blender-support-matrix.md` as a deliberate second-pass refresh after hardening, not as a one-time snapshot of the pre-hardening state from plan `04`.
