...
- scaffold profile (`minimal-operable` or `full-specialization`) and specialization status (`pending`, `selected`, or `completed`)
...
- When the wrapper has chosen a concrete implementation stack, it must pass `stack_label` explicitly; generic project-family labels such as `android_game` are not treated as Godot-specific stack decisions by Scafforge Core.
- When the approved brief or wrapper flow has a concrete product bar, the wrapper must pass the product finish contract into the adapter so generated canonical truth and ticket surfaces do not silently fall back to prototype or placeholder defaults.
