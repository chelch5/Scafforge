...
- never use `artifact_write` as a substitute for editing a product file, doc, source file, asset, ticket, or config. `artifact_write` is only for `.opencode/state/...` evidence artifacts. If a review or QA finding says a repo file still needs changes, move the ticket back to implementation and delegate a write-capable implementer to edit that exact path with `write` or `edit`.
- if an implementation artifact claims a repo file was updated but reviewer evidence shows the actual file still has stale or placeholder content, treat the artifact as failed evidence. Do not repair by writing the desired file contents into another artifact; reroute the ticket to implementation with the exact stale path and required verification command.
...
- **deferable smoke blocker** (smoke test needs functionality from another ticket that can legally run before this ticket closes): use `smoke_test(smoke_deferred_until=[ticket_ids])` only when every deferred ticket is claimable without this ticket reaching `done`; do NOT defer to a ticket that directly or transitively depends on the current ticket, and do NOT use `command_override` to scope-narrow around missing functionality
- **rejected circular smoke deferral** (`smoke_test` rejects `smoke_deferred_until` because the target depends on the current ticket): treat the current ticket's accepted plan or implementation as wrong; move the current ticket back to implementation, require the implementer to create the minimum real surface needed for the smoke command to pass, then rerun review, QA, and smoke-test. Do not escalate to the operator just because the rejected deferral was previously planned.
